...
DataSetBase<DATA, GEOM>::DataSetBase(const std::vector<util::DateTime> & times,
  ...
  : ntimes_(times.size()), localtimes_(), alltimes_(times),
    commTime_(eckit::mpi::comm(commTime.name().c_str())),
    subWinLength_(),
    nmembers_(members.size()), localmembers_(), allmembers_(members),
    commEns_(eckit::mpi::comm(commEns.name().c_str())),
    mymembers_(),
    dataset_()
{
...
DataSetBase<DATA, GEOM>::DataSetBase(const eckit::mpi::Comm & commTime,
                                     const eckit::mpi::Comm & commEns)
  : ntimes_(), localtimes_(), alltimes_(),
    commTime_(eckit::mpi::comm(commTime.name().c_str())),
    subWinLength_(),
    nmembers_(), localmembers_(), allmembers_(),
    commEns_(eckit::mpi::comm(commEns.name().c_str())),
    mymembers_(),
    dataset_()
{}
...
DataSetBase<DATA, GEOM>::DataSetBase(const DataSetBase<DATA, GEOM> & other)
  : ntimes_(other.ntimes_), localtimes_(other.localtimes_), alltimes_(other.alltimes_),
    commTime_(eckit::mpi::comm(other.commTime_.name().c_str())), subWinLength_(other.subWinLength_),
    nmembers_(other.nmembers_), localmembers_(other.localmembers_), allmembers_(other.allmembers_),
    commEns_(eckit::mpi::comm(other.commEns_.name().c_str())), mymembers_(other.mymembers_),
    dataset_()
{
...
void DataSetBase<DATA, GEOM>::sync_times() {
  ...
  // Get global size
  size_t globalSize = alltimes_.size();
  commTime_.allReduceInPlace(globalSize, eckit::mpi::sum());

  // Define local vector of strings
  std::vector<std::string> globalStrings;
  for (const auto it : alltimes_)
    globalStrings.push_back(it.toString());

  // Allgatherv vector of string
  std::pair<std::vector<char>, std::vector<size_t>> encodedX = encodeStrings(globalStrings);
  eckit::mpi::Buffer<char> charBuffer(commTime_.size());
  commTime_.allGatherv(encodedX.first.begin(), encodedX.first.end(), charBuffer);
  eckit::mpi::Buffer<size_t> lengthBuffer(commTime_.size());
  commTime_.allGatherv(encodedX.second.begin(), encodedX.second.end(), lengthBuffer);
  encodedX = {};
  globalStrings = decodeStrings(charBuffer.buffer, lengthBuffer.buffer);

  // Reset alltimes_
  alltimes_.clear();
  for (const auto it : globalStrings)
    alltimes_.push_back(util::DateTime(it));
}
