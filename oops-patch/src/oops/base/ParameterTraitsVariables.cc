...
std::vector<std::string> getVariableNamesWithoutChannelSuffix(const patch::Variables &variables) {
  if (variables.channels().empty())
    return variables.variables();

  auto throwException = [&variables] {
    std::stringstream msg;
    msg << "The patch::Variables object " << variables << " cannot be stored in a Configuration object: "
